...
const akihabaraLocations = [
  { id: 1, title: "秋葉原駅", description: "JRと日比谷線の駅", latitude: 35.69836, longitude: 139.77313, image: require("@/assets/images/sampleManholeImg/1.jpg") },
  { id: 2, title: "神田明神", description: "歴史ある神社", latitude: 35.7018, longitude: 139.7676, image: require("@/assets/images/sampleManholeImg/2.jpeg") },
  { id: 3, title: "ヨドバシAkiba", description: "大型家電量販店", latitude: 35.6986, longitude: 139.7748, image: require("@/assets/images/sampleManholeImg/3.jpeg") },
  { id: 4, title: "秋葉原ガチャポン会館", description: "たくさんのガチャガチャがある", latitude: 35.7004, longitude: 139.7707, image: require("@/assets/images/sampleManholeImg/4.jpeg") },
  { id: 5, title: "MLB 埼玉", description: "地元の野球選手デザインマンホール撮ってみた", latitude: 36.03447, longitude: 139.39882, image: require("@/assets/images/sampleManholeImg/5.jpeg") },
];
...
export default function ManholeMap() {
    ...
    const initializeLocation = async () => {
      try {
        ...
        locationSubscription = await Location.watchPositionAsync(
          ...
          (newLocation) => { if (isMounted) setLocation(newLocation); }
        );
      } catch (error) {
        console.error(error);
  ...
  const animateSheet = (height: number, opacity: number, onComplete?: () => void) => {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (e, gestureState) => bottomSheetHeight.setValue(-gestureState.dy),
      onPanResponderRelease: (e, gestureState) => {
        ...
        if (gestureState.vy > 0.5 || finalHeight < PARTIAL_VIEW_HEIGHT * 0.5) {
          closeSheet();
        } else if (gestureState.vy < -0.5 || finalHeight > PARTIAL_VIEW_HEIGHT * 1.5) {
  ...
  // --- Render ---
  if (!region) {
    return <ThemedView style={styles.loadingContainer}><Text>{errorMsg || "Getting location..."}</Text></ThemedView>;
  }
    ...
    <ThemedView style={styles.container}>
      ...
      >
        {akihabaraLocations.map((locationItem) => (
          <Marker
            key={locationItem.id}
            coordinate={{ latitude: locationItem.latitude, longitude: locationItem.longitude }}
            onPress={() => handleMarkerPress(locationItem)}
            tracksViewChanges={selectedLocation?.id === locationItem.id}
            pinColor={selectedLocation?.id === locationItem.id ? 'tomato' : 'red'}
          />
        ))}
      </MapView>
      ...
      <TouchableOpacity style={styles.currentLocationButton} onPress={() => {
        ...
        <Image source={require("@/assets/images/GPSicon.png")} style={styles.gpsIcon} />
      </TouchableOpacity>
      ...
      {displayLocation && (
        <Animated.View style={[
          ...
          <View style={{ flex: 1 }} {...panResponder.panHandlers}>
            ...
            <Animated.ScrollView
              style={[styles.contentScrollView, { opacity: animatedContentOpacity }]}
              showsVerticalScrollIndicator={false}
              scrollEnabled={isSheetOpenFully}
            >
              <>
                <Animated.Image source={displayLocation.image} style={[styles.cardImage, { height: animatedImageHeight }]} />
                <Text style={styles.cardTitle}>{displayLocation.title}</Text>
                <Text style={styles.cardDescription}>{displayLocation.description}</Text>
